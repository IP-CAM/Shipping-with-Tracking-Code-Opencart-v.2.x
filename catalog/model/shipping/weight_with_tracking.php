<?php
class ModelShippingWeightWithTracking extends Model {
	public function getQuote($address) {
		...
		foreach ($query->rows as $result) {
			if ($this->config->get('weight_with_tracking_' . $result['geo_zone_id'] . '_status')) {
			...
			if ($status) {
				...
				$rates = explode(',', $this->config->get('weight_with_tracking_' . $result['geo_zone_id'] . '_rate'));
		...
		if ($quote_data) {
			$method_data = array(
				'code'       => 'weight_with_tracking',
				'title'      => $this->language->get('text_title'),
				'quote'      => $quote_data,
				'sort_order' => $this->config->get('weight_with_tracking_sort_order'),
				'error'      => false
			);
		}
